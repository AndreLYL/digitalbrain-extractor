...
describe("LLM Providers", () => {
  describe("Mock Provider", () => {
    ...
    it("should handle multiple messages by concatenating content", async () => {
      const responses = new Map([["system instruction user prompt", "Combined response"]]);
    ...
    it("should throw error if no matching response found", async () => {
      ...
      await expect(provider.chat([{ role: "user", content: "goodbye" }])).rejects.toThrow(
        "No mock response found for prompt",
      );
    });
  ...
  describe("OpenAI Provider", () => {
    ...
    it("should construct correct request body for chat completion", async () => {
      ...
      const result = await provider.chat([{ role: "user", content: "Hello, AI!" }]);
    ...
    it("should throw error if response choice has no message", async () => {
      ...
      await expect(provider.chat([{ role: "user", content: "test" }])).rejects.toThrow(/message/i);
    });
  });
});
