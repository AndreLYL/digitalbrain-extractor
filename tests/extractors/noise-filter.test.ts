...
import type { ConversationBlock } from "../../src/core/types";
import { filterNoise } from "../../src/extractors/noise-filter";
...
describe("NoiseFilter", () => {
  ...
  describe("L2: LLM-based significance judgment", () => {
    ...
    it("should handle JSON parsing errors gracefully", async () => {
      const mockProvider = createMockProvider(new Map([["significance", "invalid json"]]));
  ...
  describe("Integration: L1 + L2", () => {
    it("should skip in L1 and never call L2 for system notifications", async () => {
      let l2Called = false;
      const _mockProvider = createMockProvider(new Map());
