...
describe("CursorStore", () => {
  ...
  it("should mark dirty after set() and clean after commit()", () => {
    ...
    const mtimeBefore = existsSync(CURSOR_PATH)
      ? require("node:fs").statSync(CURSOR_PATH).mtimeMs
      : null;
  ...
  it("should handle empty YAML file gracefully", () => {
    require("node:fs").writeFileSync(CURSOR_PATH, "", "utf-8");
  ...
  it("should handle malformed YAML gracefully", () => {
    require("node:fs").writeFileSync(CURSOR_PATH, "invalid: yaml: content:", "utf-8");
