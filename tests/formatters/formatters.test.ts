...
import type { ExtractionResult, Entity, Decision, TaskSignal, Discovery, TimelineEntry, Link, SourceRef } from '../../src/core/types';
...
describe('JSONFormatter', () => {
  ...
  it('should preserve decision data', () => {
    ...
    const jwtDecision = parsed.signals.decisions.find(
      (d: Decision) => d.summary.includes('JWT')
    );
  ...
  it('should preserve task data with status', () => {
    ...
    const inProgressTask = parsed.signals.tasks.find(
      (t: TaskSignal) => t.status === 'in_progress'
    );
